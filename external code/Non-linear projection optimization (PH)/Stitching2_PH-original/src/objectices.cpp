...
: L(L_), F(F_), projector(projector_), I(I_)
{
   ...
   // posConstraints = VP;
    ...
    for(auto& l : loops) for(int i : l) boundary[i] = 1;
    ...
    for(auto& x : invSqrtArea) x = sqrt(x / 3.);
...
double OptimizationObjectives::gaussianCurvatureObjective(const Eigen::MatrixXd& V, const double w, bool sqrtAreaNormalization)
{
    ...
    {
        for(int i = 0; i < nv; ++i)
            vals(i) *= invSqrtArea[i];
        
        if(w)
        {
            for(auto& t : trip)
            {
                t = Eigen::Triplet<double>(t.row(), t.col(), t.value() * invSqrtArea[t.row()]);
            }
        }
    }
