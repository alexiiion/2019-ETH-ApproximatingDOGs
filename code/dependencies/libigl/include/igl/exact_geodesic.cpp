...
IGL_INLINE void igl::exact_geodesic(
  ...
  std::vector<Eigen::MatrixXd> &P) //EDIT ALEX
{
  ...
  for (int i = 0; i < target.size(); i++)
  {
	...
	Eigen::MatrixXd geodesic_path(path.size(), 3);
	for (int j = 0; j < path.size(); j++)
	{
		Eigen::RowVector3d point(path[j].x(), path[j].y(), path[j].z());
		geodesic_path.row(j) = point;
	}

	P.push_back(geodesic_path);
  }
}

#ifdef IGL_STATIC_LIBRARY //NOT EDITED (ALEX) but should for consistency
