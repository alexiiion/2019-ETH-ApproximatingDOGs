...
// EDIT ALEX/MICHAEL: light mesh from both sides
...
IGL_INLINE void igl::opengl::MeshGL::init()
{
  ...
  void main()
  {
    ...
    vec4 Kdi2 = Kdi;
    if (!gl_FrontFacing) Kdi2 = vec4(0.8,0.8,0.8,1.); // add back color

    vec3 vector_to_light_eye = light_position_eye - position_eye;
    vec3 direction_to_light_eye = normalize (vector_to_light_eye);
    float dot_prod = dot (direction_to_light_eye, n);
    float clamped_dot_prod = max (dot_prod, 0.0);
    vec3 Id = Ld * vec3(Kdi2) * clamped_dot_prod;    // Diffuse intensity
    ...
    vec4 color = vec4(lighting_factor * (Is + Id) + Ia + (1.0-lighting_factor) * vec3(Kdi2),(Kai.a+Ksi.a+Kdi2.a)/3);
