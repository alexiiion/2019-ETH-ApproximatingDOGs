...
//TODO delete and use list_to_string() instead
template<typename T>
void log_list(const int log_level, const std::vector<T>& list, const std::string header = "", const bool multi_line = true, const bool print_index = false)
{
	if (header.length() > 0)
	{
		write_log(log_level) << header;

		if (multi_line)
			write_log(log_level) << std::endl;
	}

	for (int i = 0; i < list.size(); i++)
	{
		if (print_index)
			write_log(log_level) << "[" << i << "]: ";

		write_log(log_level) << list[i];
