...
void quad_topology(const Eigen::MatrixXd& V, const Eigen::MatrixXi& Fsqr, QuadTopology& quadTop) {
  ...
  quad_quad_adjacency(F, quadTop, quadTop.QQA);
...
void quad_quad_adjacency(const Eigen::MatrixXi& F, const QuadTopology& quad_topology, std::vector<std::vector<int>>& out_QQA)
{
	out_QQA.clear();
	Eigen::MatrixXi Fsqr = F_to_Fsqr(F);

	for (int i = 0; i < Fsqr.rows(); i++)
	{
		std::vector<Edge> quad_edges;
		get_edges_from_face(F, i, quad_edges);

		vector<int> all_faces;
		for (int j = 0; j < quad_edges.size(); j++)
		{
			auto faces = get_edge_faces(quad_topology, quad_edges[j]);
			all_faces.insert(all_faces.end(), faces.begin(), faces.end());
		}

		std::sort(all_faces.begin(), all_faces.end());
		all_faces.erase(std::remove(all_faces.begin(), all_faces.end(), i), all_faces.end());
		all_faces.erase(std::unique(all_faces.begin(), all_faces.end()), all_faces.end());

		out_QQA.push_back(all_faces);
	}
}

void triangle_to_quad_faces(const std::vector<int>& triangles, std::vector<int>& out_quads)
{
	const int loglevel = 4;

	auto triangles_unique = triangles;
	std::sort(triangles_unique.begin(), triangles_unique.end());
	triangles_unique.erase(std::unique(triangles_unique.begin(), triangles_unique.end()), triangles_unique.end());

	out_quads.clear();

	for (int i = 0; i < triangles_unique.size(); i++)
		out_quads.push_back(triangles_unique[i] * 0.5);

	out_quads.erase(std::unique(out_quads.begin(), out_quads.end()), out_quads.end());
}

void quad_to_triangle_faces(const std::vector<int>& quads, std::vector<int>& out_triangles)
{
	const int loglevel = 4;

	auto quads_unique = quads;
	std::sort(quads_unique.begin(), quads_unique.end());
	quads_unique.erase(std::unique(quads_unique.begin(), quads_unique.end()), quads_unique.end());

	out_triangles.clear();

	for (int i = 0; i < quads_unique.size(); i++)
	{
		out_triangles.push_back(2 * quads_unique[i]);
		out_triangles.push_back(2 * quads_unique[i] + 1);
	}
}
