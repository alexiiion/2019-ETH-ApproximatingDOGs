...
double line_search(Eigen::VectorXd& x, const Eigen::VectorXd& d, double& step_size, Objective& f, int max_iter) 
{
	...
	int cur_iter = 0; 
	int MAX_STEP_SIZE_ITER = min(max_iter,20);
	...
	while (new_energy >= old_energy && cur_iter < MAX_STEP_SIZE_ITER)
	{
		...
		write_log(DataModel::log_level_optimization) << "   --> current energy: " << cur_e << std::endl << std::endl;
		//write_log(DataModel::log_level_optimization) << "   --> current energy: " << cur_e << std::endl << "   --> elapsed time:   " << timer.getElapsedTime() - t << std::endl << std::endl;
