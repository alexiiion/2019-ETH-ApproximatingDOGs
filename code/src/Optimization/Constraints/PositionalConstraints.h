...
public:
	...
	virtual PositionalConstraints* clone() const 
	{
		return new PositionalConstraints(*this);
	}

	virtual Eigen::VectorXd Vals(const Eigen::VectorXd& x) const 
	{
		Eigen::VectorXd constrained_pts_coords; 
		igl::slice(x,b,1, constrained_pts_coords);
		
		// A vector of x(b)-bc for the constraints x(b)-bc = 0
		auto vals = constrained_pts_coords - bc;
		write_log(DataModel::log_level_optimization) << "    PositionalConstraints norm: " << vals.norm() << std::endl;
