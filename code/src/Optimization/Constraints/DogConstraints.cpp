...
Eigen::VectorXd DogConstraints::Vals(const Eigen::VectorXd& x) const {
	...
	//DataModel::output_DOG_objective = constVals.cwiseAbs().sum();
	DataModel::output_DOG_objective = constVals.norm();
	write_log(DataModel::log_level_optimization) << "    DogConstraints sum: " << DataModel::output_DOG_objective << std::endl << std::endl;
