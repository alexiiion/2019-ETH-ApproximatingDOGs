...
Eigen::VectorXd LengthRegularizerConstraints::Vals(const Eigen::VectorXd& x) const {
  ...
  DataModel::output_regularizer_energy = constVals.cwiseAbs().sum() / constVals.rows();
  write_log(DataModel::log_level_optimization) << "    LengthRegularizerConstraints norm: " << DataModel::output_regularizer_energy << std::endl;
